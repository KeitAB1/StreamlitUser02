...
if df is not None:
    ...
    class SA_with_ContainerLoading:
        def __init__(self, initial_temperature, cooling_rate, min_temperature, max_iterations, lambda_1, lambda_2,
                     lambda_3, num_positions, dataset_name, objectives, use_adaptive):
            self.initial_temperature = initial_temperature
            self.cooling_rate = cooling_rate
            self.min_temperature = min_temperature
            self.max_iterations = max_iterations
        ...
        def evaluate(self, position):
            try:
                space_utilization_score = self.objectives.maximize_space_utilization(position)
                gravity_deviation_score = self.objectives.minimize_center_of_gravity_deviation(position)
                weight_penalty = self.objectives.enforce_weight_limit(position)

                # 计算最终得分，越小越好
                score = (
                        self.lambda_1 * space_utilization_score +
                        self.lambda_2 * gravity_deviation_score +
                        self.lambda_3 * weight_penalty
                )
                return score
            except Exception as e:
                return np.inf
        ...
        def optimize(self):
            initial_position = np.random.randint(0, self.num_positions, size=len(self.objectives.items))
            return self.optimize_from_position(initial_position)
