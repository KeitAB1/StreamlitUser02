...
if df is not None:
    ...
    class ManualStackingOptimized:
        def __init__(self, plates, heights, batches, Dki, area_positions, objectives):
            """
            plates: 钢板数据，包括长度、宽度、厚度、批次等
            heights: 每个堆垛的当前高度
            batches: 钢板的批次信息
            Dki: 堆垛位置信息
            area_positions: 库区位置信息
            objectives: 优化目标，用于平衡和库区均衡计算
            """
            self.plates = plates
            ...
            self.objectives = objectives
            self.num_positions = len(Dki)

            # 检查批次和 Dki 的大小是否一致
            assert len(self.batches) == len(self.plates), "Mismatch between number of batches and plates."
            assert len(self.heights) == self.num_positions, "Mismatch between heights and Dki positions."

            # 创建一个批次位置的缓存，以便快速查找
            self.batch_position_cache = self.build_batch_position_cache()

        def build_batch_position_cache(self):
            """
            创建一个批次位置缓存，用于快速定位某个批次在哪些堆垛上已经有钢板
            """
            batch_position_cache = {}
            for position, batch in enumerate(self.batches):
                if batch not in batch_position_cache:
                    batch_position_cache[batch] = []
                if position < len(self.heights):  # 确保位置索引不超出范围
                    batch_position_cache[batch].append(position)
            return batch_position_cache

        def find_best_stack_for_batch(self, batch):
            """
            在当前批次中找到最低的堆垛位置，优先选择同一批次的堆垛。
            如果批次中没有堆垛，或者批次索引超出范围，则返回最低的堆垛。
            """
            if batch in self.batch_position_cache:
                # 只考虑在有效范围内的堆垛位置
                batch_positions = [idx for idx in self.batch_position_cache[batch] if idx < len(self.heights)]

                if batch_positions:
                    best_position = min(batch_positions, key=lambda idx: self.heights[idx])
                else:
                    st.warning(f"Batch {batch} has no valid positions, choosing lowest stack.")
                    best_position = np.argmin(self.heights)
            else:
                # 如果该批次没有对应的堆垛位置，则选择当前最低的堆垛
                st.warning(f"Batch {batch} not found, choosing lowest stack.")
                best_position = np.argmin(self.heights)

            return best_position

        def stack_plate(self, plate, batch):
            """
            将钢板堆放在最优的堆垛位置，按照批次优先和高度优先的原则
            """
            # 找到最佳堆垛位置
            best_position = self.find_best_stack_for_batch(batch)

            # 堆放钢板，更新堆垛的高度
            if best_position < len(self.heights):
                self.heights[best_position] += plate[2]  # plate[2] 为钢板的厚度
            else:
                st.error(
                    f"Best position {best_position} is out of bounds for heights array of size {len(self.heights)}.")
                return None  # 错误处理

            # 更新批次缓存
            if batch not in self.batch_position_cache:
                self.batch_position_cache[batch] = []
            self.batch_position_cache[batch].append(best_position)

            return best_position

        def run(self):
            """
            开始堆垛钢板，并返回堆垛的最终位置和高度
            """
            plate_positions = []
            for i, plate in enumerate(self.plates):
                batch = self.batches[i]
                position = self.stack_plate(plate, batch)
                if position is not None:
                    plate_positions.append(position)
                else:
                    st.error(f"Failed to stack plate {i} from batch {batch} due to invalid position.")

            return plate_positions, self.heights


    if selected_algorithm == "Manual Stacking":
        # 运行优化后的人工堆垛算法
        manual_stacking_optimized = ManualStackingOptimized(
            plates=plates,
            heights=heights,
            batches=batches,
            Dki=Dki,
        ...
        manual_positions, manual_heights = manual_stacking_optimized.run()

        st.write("Manual Stacking Results (Optimized):")
        st.write("Final Heights:", manual_heights)
        st.write("Plate Positions:", manual_positions)
